...
-- Restore Diagnostic Scan after Road Test (the OBD readiness block remains removed).
...
with diagnostic(label, sort_order) as (
  ...
  select d.*, (select coalesce(max(item_number), 0) from checklist_items) +
    row_number() over (order by d.sort_order) as item_number
  from diagnostic d
)
insert into checklist_items (section_id, item_number, label, description, sort_order)
